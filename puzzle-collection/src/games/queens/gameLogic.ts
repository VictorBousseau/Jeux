...
export function generateLevel(size: number = 8, seed?: string): Level {
    // Setup RNG
    let rng = Math.random;
    if (seed) {
    ...
    while (attempts < 20000) {
        ...
        const queens = generateValidQueens(size, rng);
        if (!queens) continue;
...
function generateValidQueens(size: number, rng: () => number): { row: number; col: number }[] | null {
...
function generateRegions(size: number, queens: { row: number; col: number }[], rng: () => number): number[][] {
